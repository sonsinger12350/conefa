...
class ITWPC_WebP_Converter
{

	/**
	 * Instance of this class
	 *
	 * @var ITWPC_WebP_Converter
	 */
	...
	public function convert_attachment_to_webp($metadata, $attachment_id)
	{
		...
		if (get_post_meta($attachment_id, '_itwpc_processing', true)) return $metadata;
		...
		// Mark as processing
		update_post_meta($attachment_id, '_itwpc_processing', true);
		...
		delete_post_meta($attachment_id, '_itwpc_processing');
	...
	public function convert_image_to_webp($file_path, $quality = 82, $max_width = null, $max_height = null)
	{
		...
		if (extension_loaded('imagick') && class_exists('Imagick')) {
			try {
				...
				error_log('ITWPC ImageMagick error: ' . $e->getMessage());
			}
		}
