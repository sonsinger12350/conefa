...
class SWC_Admin_Settings {
	...
	public function register_settings() {
		register_setting( 'swc_settings', 'swc_options', array( $this, 'sanitize_options' ) );
		...
		add_settings_field(
			...
			array( $this, 'render_max_dimensions_field' ),
			'smart-webp-converter',
			'swc_general_section'
		);
		...
		add_settings_field(
			...
			array( $this, 'render_serve_webp_field' ),
			'smart-webp-converter',
			'swc_general_section'
		);
		...
		add_settings_field(
			...
			array( $this, 'render_delete_original_field' ),
			'smart-webp-converter',
			'swc_general_section'
		);
	}
		...
		<div class="wrap">
			...
			<?php if ( $webp_supported ) : ?>
				...
				<div class="swc-batch-section" style="margin-top: 30px; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
					...
					<p><strong><?php esc_html_e( 'Total images to process:', 'smart-webp-converter' ); ?></strong> <span id="swc-total-images"><?php echo esc_html( $total_images ); ?></span></p>
	...
	public function enqueue_admin_scripts( $hook ) {
		...
		wp_enqueue_script( 'swc-admin', SWC_PLUGIN_URL . 'assets/js/admin.js', array( 'jquery' ), SWC_VERSION, true );
		wp_localize_script( 'swc-admin', 'swcAdmin', array(
